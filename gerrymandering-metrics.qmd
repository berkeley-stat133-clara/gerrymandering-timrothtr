...
write_csv(congress, "data/district_results_2024.csv")
write_csv(district_results_ab604, "data/district_results_ab604.csv")

metrics_summary <- tibble(
  map_version = c("2024 Actual", "AB604 Proposed"),
  mean_median_diff = c(mm_diff, mm_diff_ab604),
  efficiency_gap = c(efficiency_gap, eff_gap_ab604),
  prop_dem_wins = c(
    mean(congress$dem_share > 0.5, na.rm = TRUE),
    mean(district_results_ab604$dem_share > 0.5, na.rm = TRUE)
  )
)
write_csv(metrics_summary, "data/metrics_summary.csv")
```
